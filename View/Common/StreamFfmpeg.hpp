...
public:
	...
	int Initialize(std::string outFileName, int width, int height, int fps, int channelCount, 
		AVSampleFormat sampleFmt, int sampleRate, int audioLag, bool toFile, int videoCodec, int audioCodec, int crf = 23);
	int Init_Audio_Resampler(int channelCount, AVSampleFormat sampleFmt, int sampleRate);
	int Init_Audio_Fifo();

	int StreamFrame( uint8_t* input, int frame_index, int width, int height );
