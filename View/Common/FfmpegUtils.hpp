...
static int add_stream(OutputStream *ost, AVFormatContext *oc, AVCodec **codec, enum AVCodecID codec_id,
	int width, int height, int video_fps, bool toFile, int audioSampleRate, int channels)
{
	AVCodecContext *c;
	int i;
...
static AVFrame *get_video_frame( OutputStream *ost, uint8_t* data )
{
	...
	else */
	{
		if (data)
		{
			...
			if (c->pix_fmt == AV_PIX_FMT_YUV420P)
			{
				SwsContext * ctx = sws_getContext(c->width, c->height, AV_PIX_FMT_BGR24, c->width, c->height, FILE_PIX_FMT, 0, 0, 0, 0);
				uint8_t* inData[1];
				int inLineSize[1];

				inData[0] = data;		// RGB24 has only one plane
	...
	if (ost->next_pts + 100 < ost->now_time)
	{
		std::cout << "S";
...
static int ffmpeg_stream_count = 0;

static int write_video_frame( AVFormatContext *oc, OutputStream *ost, uint8_t* data, int frameIndex, int fps )
{
	int ret;
	AVCodecContext *c;
	AVFrame *frame;
