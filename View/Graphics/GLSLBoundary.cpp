...
GLSLBoundary::GLSLBoundary(QObject *parent) : GLSLProgram(parent)
{
}
...
void GLSLBoundary::initialize(int panoWidth, int panoHeight, int viewCount, int viewIdx)
{
	...
	m_program->setUniformValue("currentIdx", viewIdx);
...
void GLSLBoundary::render(GLuint fbos[])
{
	m_program->bind();
	m_gl->glBindFramebuffer(GL_FRAMEBUFFER, m_fboId);
...
const int GLSLBoundary::getWidth()
{
	return panoramaWidth;
}

const int GLSLBoundary::getHeight()
{
