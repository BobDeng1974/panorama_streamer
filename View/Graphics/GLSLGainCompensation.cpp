...
GLSLGainCompensation::GLSLGainCompensation(QObject *parent) : GLSLProgram(parent)
{
}
...
void GLSLGainCompensation::initialize(int cameraWidth, int cameraHeight)
{
	...
	m_gl->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, cameraWidth, cameraHeight, 0, GL_BGRA, GL_UNSIGNED_BYTE, NULL);
	...
	QMatrix4x4 matrix;
	matrix.ortho(-cameraWidth / 2, cameraWidth / 2, -cameraHeight / 2, cameraHeight / 2, -10.0f, 10.0f);
...
void GLSLGainCompensation::render(GLuint textureId, float ev)
{
	m_program->bind();
	m_gl->glBindFramebuffer(GL_FRAMEBUFFER, m_fboId);
...
const int GLSLGainCompensation::getWidth()
{
	return cameraWidth;
}

const int GLSLGainCompensation::getHeight()
{
	return cameraHeight;
}