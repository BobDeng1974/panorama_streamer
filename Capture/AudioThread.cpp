...
AudioThread::~AudioThread()
{
	if (cap != NULL)
		disconnect();
	stopAudioThread();
	if (audioThreadInstance) delete audioThreadInstance;
}
...
void AudioThread::initialize(SharedImageBuffer *sharedImageBuffer, int deviceIndex)
{
	...
	m_audioDeviceName = sharedImageBuffer->getGlobalAnimSettings()->cameraSettingsList()[m_deviceNumber].audioName;
...
void AudioThread::run()
{
    ...
    while( 1 )
    {
		...
		if (frame && sharedImageBuffer->getStreamer()) {
			sharedImageBuffer->setAudioFrame((void*)frame);
...
bool AudioThread::connect()
{
	...
	QString deviceName;
	{
		...
		vector<wstring> videoDevicesW, audioDevicesW;
		...
		for (int i = 0; i < videoDevicesW.size(); i++)
			videoDevices.push_back(getStringFromWString(videoDevicesW[i]));
		for (int i = 0; i < audioDevicesW.size(); i++)
			audioDevices.push_back(getStringFromWString(audioDevicesW[i]));

		T_LOG("Video Devices");
		for (int i = 0; i < videoDevices.size(); i++)
			std::cout << videoDevices[i] << endl;

		T_LOG("Audio Devices");
		for (int i = 0; i < audioDevices.size(); i++)
			std::cout << audioDevices[i] << endl;

		deviceName = this->sharedImageBuffer->getGlobalAnimSettings().m_audioDeviceName;
		for (int i = 0; i < audioDevices.size(); i++)
		{
			const char * realDeviceName = audioDevices[i].c_str();
			if (deviceName == realDeviceName)
				this->m_deviceNumber = i;
		}
	}

	if (m_deviceNumber == -1)
	{
		T_LOG("No microphone selected");
		return false;
	}
