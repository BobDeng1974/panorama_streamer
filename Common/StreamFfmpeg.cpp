...
int StreamFfmpeg::Initialize(std::string outFileName, int width, int height, int fps, int channelCount, AVSampleFormat sampleFmt, 
	int sampleRate, int audioLag, bool toFile, int videoCodec, int audioCodec, int crf)
{
	pts = 0;
	...
	if (outFileName.substr(0, 4) == "rtsp")
		...
		avformat_alloc_output_context2(&formatContext_, NULL, NULL, out_filename);
	if( !formatContext_ ) 
	{
	...
	if( outputFormat_->video_codec != AV_CODEC_ID_NONE ) 
	{
		...
		ret = add_stream(&mOutputVideoStream, formatContext_, &mVideoCodec, outputFormat_->video_codec,
			width, height, fps, toFile, sampleRate, channels);
		if (ret < 0)
			return ret;
		mHaveVideo		= 1;
		mEncodeVideo	= true;
	...
	{	
		...
		Init_Audio_Resampler(channels, m_inSampleFmt, sampleRate);
		Init_Audio_Fifo();
	}
...
int StreamFfmpeg::StreamFrame( uint8_t* input, int frame_index, int width, int height )
{
	...
	//while( encode_video || encode_audio ) 
	{
		...
		//if (mEncodeVideo) {
			//mEncodeVideo = !write_video_frame(formatContext_, &mOutputVideoStream, input, frame_index);
		streamMutex.lock();
		write_video_frame(formatContext_, &mOutputVideoStream, input, frame_index, videoFps);
		streamMutex.unlock();
		//}
		//else {
